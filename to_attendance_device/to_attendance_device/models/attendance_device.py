...
class AttendanceDevice(models.Model):
    ...
    def connect(self):
        ...
        try:
            ...
            error_msg = _(" to the device '%s'. Please check your network"
...
def action_attendance_download(self):
    ...
    device = self.env['attendance.device'].search([('state', '=', 'confirmed')], limit=1)
...
def get_dict(self, ci, attendance_custom, stat):
    utc_timestamp = self.convert_time_to_utc(ci.timestamp, self.env.user.tz)
    str_utc_timestamp = fields.Datetime.to_datetime(fields.Datetime.to_string(utc_timestamp))

    local_time = self.get_local_time(ci.timestamp)
    return {
        'timestamp': ci.timestamp,
        'check': str(self.get_time(ci.timestamp)),
    ...
    if attendance_custom and checks:
        ...
        if len(checks) == 0:
            ...
            if len(checks) > 1:
                ...
                for ci in checks:
                    if fcin != None:
                        ...
                        if minutes < 1:
                            ...
                            rcord = self.get_dict(ci, attendance_custom, type)
                            moves += self.create_instance(rcord, stat)
                            if stat == 0:
                                stat = 1
                                type = 'checkout'
                            else:
                ...
                rcord = self.get_dict(checks[0], attendance_custom, type)
                moves += self.create_instance(rcord, stat)
    return moves
...
def elimnate_seconds_diff_records(self, checks):
    elimnated = []
    if len(checks) > 1:
        fcin = None
        for ci in checks:
...
def action_create_attendance(self):
    ...
    for rec in get_emp:
        if rec.resign or rec.terminate or rec.runaway:
            pass
        else:
            ...
            limit = int(len(time_stams) / 2)
            # if limit>3:
            #     limit =int(limit/2)
            time_stams = time_stams[-30:]
            last_time = -1
            ...
            if time_stams:
                ...
                # check = self.env['user.attendance'].search(
                #     [('employee_id', '=', rec.id),
                #      ], order='timestamp asc')
                #
                while start_date <= end_date:
                    ...
                    # checks = check.filtered(lambda
                    #                             r: r.timestamp.day == date.day and r.timestamp.month == date.month and r.timestamp.year == date.year).sorted(
                    #     key=lambda l: l.timestamp)

                    checks = self.env['user.attendance'].with_context(
                        tz=tz or self.env.user.sudo().tz or 'UTC').search(
                        [('employee_id', '=', rec.id), ('timestamp', '>=', str(date) + ' 00:00:00'),
                         ('timestamp', '<=', str(date) + ' 23:59:59')], order='timestamp asc')

                    # checks_barcode = self.env['barcodes.attendance'].with_context(
                    #     tz=tz or self.env.user.sudo().tz or 'UTC').search(
                    #     [('employee_id', '=', rec.id), ('timestamp', '>=', str(date) + ' 00:00:00'),
                    #      ('timestamp', '<=', str(date) + ' 23:59:59')], order='timestamp asc')

                    # strt=datetime.strptime(str(date) + ' 00:00:00','%Y-%m-%d %H:%M:%S')
                    # endd=datetime.strptime(str(date) + ' 23:59:59','%Y-%m-%d %H:%M:%S')
                    #
                    #
                    # checks_wc = self.env['user.attendance'].search(
                    #     [('employee_id', '=', rec.id)], order='timestamp asc').filtered(lambda r:r.timestamp >= strt and
                    #                                                                                     r.timestamp <= endd)
                    #
                    # if len(checks) != len(checks_wc):
                    #     print("LEN CHANGES")

                    checks = self.elimnate_seconds_diff_records(checks)
                    # check_barcode=self.elimnate_seconds_diff_records(checks_barcode)

                    # if check_barcode and not checks:
                    #     project=check_barcode[0].project_id
                    #     checks=check_barcode
                    #     is_barcode_attend=True

                    if len(checks) > 4:
                        last_check = checks[len(checks) - 1]
                        checks = checks[:3]
                        checks += last_check
                    if len(checks) in [3, 0]:
                        is_modif = True
                        state = 'Modify'
                    ...
                    if not is_exist:
                        is_exist = attendance_custom = self.env['attendance.custom'].create(mydic)
                        self.chk_otmax(is_exist, start_date, delta_p, rec)

                        # raise ValidationError(_("There the isse %s" + str(is_exist)))

                        self.create_custom_records(attendance_custom, checks, date)
                        # check last record
                        ...
                        for dec in is_exist:
                            tot_len = 0
                            if dec.is_modif:
                                for red in dec.check_ins:
                                    red.unlink()
                                for reg in dec.check_outs:
                                    reg.unlink()
                                self.create_custom_records(dec, checks, date)
                            else:
                                if dec.check_ins:
                                    tot_len += int(len(dec.check_ins) or 0)
                                if dec.check_outs:
                                    tot_len += int(len(dec.check_outs) or 0)
                                if (tot_len != len(checks) and len(checks) != 0):
                                    for red in dec.check_ins:
                                        red.unlink()
                                    for reg in dec.check_outs:
                                        reg.unlink()
...
def chk_otmax(self, is_exist, start_date, delta_p, rec):
    last_date = start_date - delta_p
    is_exist_last = self.env['attendance.custom'].search(
        [('employee_id', '=', rec.id), ('attendance_date', '=', last_date)], limit=1)
    if (is_exist_last and is_exist) and is_exist.id not in [False, None]:
        if is_exist_last.ot_125 and is_exist_last.ot_125 not in [False, None]:
            if int(str(is_exist_last.ot_125).split(':')[0]) > 7:
                if is_exist and is_exist.id:
                    try:
                        is_exist.is_bonus_day = True
                        is_exist.valid = True
                        is_exist.state = 'Validated'
                        is_exist.is_modif = False
                        is_exist.absent = False
                    except:
                        _logger.info("%s partners updated installing module.", str(is_exist_last))


# def calc_total_working_hours(self, check):
#     seconds = 0
#     if check:
#         for inc, out in zip(check):
#             diff = out.timestamp - inc.timestamp
#             seconds += diff.seconds
#
#     return self.get_minute_hmformat(seconds)

def create_custom_records(self, attendance_custom, checks, date):
    # attendance_custom.employee_id.workschedule='08:00-13:00|14:00-17:00'
    check = self.create_checkins_outs(attendance_custom, checks)
...
def set_records_scheduler(self, id_no=None):
    emp = self.env['hr.employee'].search([('identification_id', '=', id_no)])
    get_emp = self.env['user.attendance'].search([('employee_id', '=',)])
    for rec in get_emp:
        pass
