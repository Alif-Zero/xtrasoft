        ...
        for rec in dept:
            ...
            if emps:
                ...
                if atten:
                    dix = {}
                    dix['data'] = 'd'
                    dix['div'] = rec.name if not self.select_emp else emps.contract_id.grade.department.name
                    plist.append(dix)
                    delta = timedelta(days=1)
                    start_date = self.start_date
                    end_date = self.end_date
                            ...
                            if dec.date_of_join < start_date:
                                ...
                                if attends:
                                    if ot == 1 and ((int(str(attends.ot_125 or '00:00').split(':')[0],0) or 0 > 0) or (int(str(attends.ot_15 or '00:00').split(':')[0],0) or 0 > 0)):
                                        ...
                                        dix['erly_in'] = 'Valid' if check else 'InValid'
                                        plist.append(dix)
                                        ...
                                        if attends.absent:
                                            ...
                                            statex = 'Present'
                                        dix = {}
                                        dix['titlex']='MONTHLY ATTENDANCE REPORT VALID'
                                        dix['roll'] = dec.identification_id
                                        dix['emp'] = dec.name
                                        dix['dept'] = dec.contract_id.grade.department.name
                                        dix['state'] = statex
...
class dailyxtscxReport_valid(models.AbstractModel):
    _name = 'report.attendance_customization.valid_attend_details'
    _description = "Emp logs Report"
